...
def collapse_subj_num(subj, num):
    num_pattern = "^(0{1,3})"
    num = re.sub(num_pattern, "", num)
    num = re.sub(" ", "", num)
    collapsed = subj + " " + num
    return collapsed
parsed["subj_cat"] = [collapse_subj_num(parsed.loc[i, "subj_area_cd"], parsed.loc[i, "crs_catlg_no"]) for i in range(len(parsed))]
