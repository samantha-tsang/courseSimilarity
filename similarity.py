...
### import dataset
parsed = pd.read_csv("preprocessed_parsed.csv")
### eliminate courses with no description
parsed = parsed[~parsed["extra_clean"].isnull()]
...
for i in range(len(parsed)):
    parsed.loc[i, "extra_clean"] = course_type_remove(parsed.loc[i, "extra_clean"])
    parsed.loc[i, "extra_clean"] = course_num_remove(parsed.loc[i, "extra_clean"])
    parsed.loc[i, "extra_clean"] = course_hours_remove(parsed.loc[i, "extra_clean"])
    #repeat for courses with two class types and two different hours(ie, lecture and discussion)
    parsed.loc[i, "extra_clean"] = course_type_remove(parsed.loc[i, "extra_clean"])
    parsed.loc[i, "extra_clean"] = course_hours_remove(parsed.loc[i, "extra_clean"])
parsed.to_csv("parsed.csv")
...
def preprocess(text):
    ...
    return result
processed_desc = parsed["extra_clean"].map(preprocess)
...
parsed["Tag ID"] = range(0, 14500) #add column to represent corresponding tag id for each document
parsed.to_csv("parsed.csv")
